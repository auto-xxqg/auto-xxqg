            ...
            <viewpager id="viewpager">
                ...
                <frame>
                    ...
                    <vertical>
                        ...
                        <pref-checkbox id="update" text="关闭自动检查更新和更新题库"/>
...
ui.v300.setText("v3.0.0:2021.8.14\n1，四人赛第一局不答，遇到有来源的题目不会卡住\n2，每周专项添加至默认执行\n3，修复听广播问题\n4，修复切换账户执行的文章的问题\n");
ui.v270.setText("v2.7.0:2021.8.1\n1，添加专项和每周答题\n2，修复一些bug\n3，个别执行更新\n4，ui界面更新\n");
...
ui.emitter.on("options_item_selected", (e, item)=>{
    switch(item.getTitle()){
        case "协议":
            ...
            break;
        case "关于":
            alert("关于", "自动学习强国 v3.0.0 正式版，auto-xxqg开发团队制作");
            break;
        case "说明":
            alert("使用说明",
...
ui.save.click(function () {
    ...
    zsyNext = parseInt(ui.zsyNext.getText());
    zsyWaitTime = parseInt(ui.zsyWaitTime.getText());
    var ud = "http://xzy.center/web/xxqg/v.html";
...
function zsyQuestion() {
    ...
    while (true) {
       ...
       if(zNum >= zCount-1 && zNum <= zsyNext+zCount){
           log("等待"+(4+zsyWaitTime)+"秒后作答--延时作答");
           delay(4+zsyWaitTime);//4秒是等待题目加载出
       }
       if(zNum != 0){
           zsyQuestionLoop();
       }else{
           if(zT == 0){
           log("第一局容易匹配到挂，所以不答，不会减分");
           zT = 1;
           }
           toast("防止息屏呀，awa");
        }
 ...
 try{//20201025使用try catch(e)语句处理错误，去除前后置0.5s延时   
  ...
  if (!className("RadioButton").exists() || className("android.view.View").text("继续挑战").exists() || textContains("继续挑战").exists() /*|| !textContains("距离答题结束").exists()*/){//不存在本局结束标志 继续挑战，则执行  20201225界面变化，距离答题结束 删除
    ...
    if (className("RadioButton").exists() || aquestion.length == 0) {
        ...
        if(question == ""){
            var aquestion = className("android.view.View").findOne().parent().child(0).text();
            var question = aquestion.substring(3); //争上游和对战题目前带1.2.3.需去除
            log.error(question);
        }
        //以下循环为立马振山河修改
        while (aquestion == oldaquestion || question == "") {
         ...
         return;
            ...
            aquestion = className("ListView").findOnce().parent().child(1).text();
            question = aquestion.substring(3);
            console.error("提取到来源题目了");
        }else{
         aquestion = className("ListView").findOnce().parent().child(0).text();
         question = aquestion.substring(3);
         console.error("提取题目ing");
         } 
        }
      }else {
...
function updateToServer(question,answer) {
    console.info("开始上传");
    var res = http.post("http://tiku.xzy.center/insertOrUpdate",

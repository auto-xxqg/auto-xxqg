...
var r = http.get(conf[7]);
//log(r);
if(r.statusCode != 200){ 
    log("请求更新失败: " + r.statusCode + " " + r.statusMessage); 
}else{
var update = r.body.json();
if (update.v != (app.verionName)) {
    alert("检测到有更新 "+update.v, "检测到新版本，请前往http://xzy.center/web/xxqg/下载新版本！\n版本号："+update.v+"\n更新简介:\n"+update.mes);
}else{
    log("已是最新版本");
}
}
...
function updateTikunet() {
    log("开始更新题库，请先不要进行操作!!!");
    log("开始下载题库json数据...");
    var tikuServer = http.get("http://tiku.xzy.center/getAnswerByQuestion");
if(tikuServer.statusCode != 200){ 
    ...
    var iff = parseInt(files.read("./if.txt"));
